...
export const updateLoan = async (req, res) => {
  try {
    if (req.user.user_type !== "admin") throw "You are not authorized to approve loans.";
...
export const repayLoan = async (req, res) => {
  try {
    ...
    if (!loanId || !installmentId || !amount) throw "Incomplete repayment details.";
    ...
    if (!repayment || repayment.status === "paid") throw "Invalid repayment installment.";

    if (amount < repayment.amount) throw "Repayment amount is less than scheduled amount.";
